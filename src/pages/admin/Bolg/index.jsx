...
function Blog() {
  ...
  return (
    <Container>
      <Row  className="forheight scroll">
        ...
        {isLoading ? (
          ...
          data?.map((item) => (
            <Col
              key={item.uudi}
              className="mt-4 col-12 col-md-6 col-lg-4 shadow-sm p-4 rounded-4 shadow"
            >
              ...
              <div>
                <h6 className="fw-bold mt-4">{item.heading}</h6>
                <p
                  className="mt-3"
                  style={{
                    height: showMore === item.uudi ? "100%" : "75px",
                    textWrap: "wrap",
                    overflow: "hidden",
                  }}
                >
                ...
                <p
                  ...
                  onClick={() => {
                    setShowMore(showMore === item.uudi ? null : item.uudi);
                  }}
                  style={{ cursor: "pointer" }}
                >
                  Show {showMore === item.uudi ? "Less" : "More"}
                </p>
              </div>
              <p className="text-secondary">
                {item.eventDate} - {item.userName}
              </p>
              <Button
                className="mb-5"
                onClick={() => {
